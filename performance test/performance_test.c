...
static int wrong = 0;
static const char* validation_dir = "..\\data\\jsonchecker\\*.*";
static const char* roundtrip_dir = "..\\data\\roundtrip\\*.*";
static const char* performance_dir = "..\\data\\performance\\*.*";
...
#define IS_PASS_TEST(file) file.cFileName[0] == 'p'
...
static char* read_json(char* filename, const char* dirname){
	char* fullpath = (char*)malloc(strlen(dirname) + strlen(filename));
	memcpy(fullpath, dirname, strlen(dirname) - 3);
	fullpath[strlen(dirname)-3] = '\0';
...
static void test_validation() {
	...
	HANDLE fh = FindFirstFile(validation_dir, &file);
	printf("validation test\n");
	if (INVALID_HANDLE_VALUE == fh) {printf("could not find data!\n"); return;}
    lept_value v;
	correct = 0;
	wrong = 0;
    do{
        ...
        lept_init(&v);
        char* json = read_json(file.cFileName, validation_dir);
        if(IS_PASS_TEST(file)){
            if(lept_parse(&v, json)!=LEPT_PARSE_OK || (!(v.type==LEPT_ARRAY || v.type==LEPT_OBJECT))){
                printf("validation test: %s validation failed\n", file.cFileName);
                wrong++;
            }
            else
                correct++;
    ...
    printf("validation test result:\ncorrect:%d\nfailed:%d\n------------------\n",correct,wrong);
}

static void test_roundtrip() {
	WIN32_FIND_DATA file;
	HANDLE fh = FindFirstFile(roundtrip_dir, &file);
	...
	do {
		if (file.cFileName[0] == '.') continue;
		lept_init(&v);
		char* json = read_json(file.cFileName, roundtrip_dir);
		char* stringify;
		if (lept_parse(&v, json) == LEPT_PARSE_OK && strcmp(stringify=lept_stringify(&v,NULL),json)==0){
			correct++;
		}
		else {
			printf("%s roundtrip failed\n", file.cFileName);
			wrong++;
		}
		free(stringify);
		free(json);
	...
	printf("roundtrip test result:\ncorrect:%d\nfailed:%d\n------------------\n", correct, wrong);
}

static void test_performance() {
	WIN32_FIND_DATA file;
	HANDLE fh = FindFirstFile(performance_dir, &file);
	...
	do {
		if (file.cFileName[0] == '.') continue;
		lept_init(&v);
		char* json = read_json(file.cFileName, performance_dir);
		QueryPerformanceCounter(&t1);
		res = lept_parse(&v, json);
		QueryPerformanceCounter(&t2);
		double time = (t2.QuadPart - t1.QuadPart) * 1000.0 / ticksps.QuadPart;
		if (res == LEPT_PARSE_OK) {
			printf("%s parse successed\nparse time: %lf \n", file.cFileName, time);
			correct++;
		}
		else {
